...
這個 repo 把團隊的最佳實踐固化成**一條指令就能裝起來**的標準配置。安裝指令是 **`coderigup`**：
...
| `settings.json` | `opencode.json`（只寫 `instructions` 欄位，其他都讓使用者自己管） | ✅ 使用者已有 `opencode.json` 時改寫到 sidecar `opencode.coderigup.json` |

**`--target all` + AGENTS.md 注意事項：** opencode 與 Claude Code 都會讀專案根目錄的 `AGENTS.md`。為了不互相覆蓋，每個 target 寫自己的 marker 區段（Claude 用 `<!-- coderigup:start -->`、opencode 用 `<!-- coderigup:opencode:start -->`），兩段在同一個檔案裡並存。
